    ...
    <div style="background-color: #fff;">
      ...
      <Modal v-model="checklistModal" width="80%">
        ...
        <div ref="checkListPrint">
          ...
          <div v-for="users in currentCheckList.groups">

            <h2 style="text-align:center">{{currentLocation.user_location_name}}考点 - 第{{currentCheckList.user_room_no}}考场</h2>
            <div class="header-mark" style="margin-top: 20px; margin-bottom: 20px;">
              <div class="remark" style="float: left;">
                <p></p>
                <p>注: 1、考生留验由考生填写姓名，监考人员认真核对；</p>
                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、此表为准考证存根和考生留验单，在资格复审时查验使用。</p>
              </div>
              <div class="teacher-sign" style="float: right;">
                <p>监考人员签字：________________</p>
              </div>
            ...
            <div class="wrapper">
              ...
              <div v-for="(item, i) in users" style="width: 20%;float: left;">
                
                <div class="item" style="margin:2px;padding: 5px;border: 1px solid #ccc;">
                  <div class="up">
                    <div class="headimg" style="float: left;padding-right: 5px;">
                      <img :src="item.user_pic" width="50" height="65">
                    </div>
                  ...
                  <div class="down">
                    ...
                    <p>准考证号:{{item.user_admission_no}}</p>
                    <p>身份证号:</p>
                    <p>{{item.user_idcard}}</p>
                    <p>岗位(代码):{{item.user_pos_name}}</p>
                  </div>
                </div>
        ...
        <Card :bordered="false" style="margin-bottom: 15px;" v-for="item in user_list">
          <Row :gutter="16">
            ...
            <Col span="3" v-for="room in item.room_users">
              ...
              <div v-for="users in room.groups">
                ...
                <div class="room_wrapper">
                  <div class="room">
                    ...
                    <div class="wrapper">
                      <div v-for="(user, i) in users" style="width: 20%;float: left">
                        <div class="item" style="margin:2px;padding: 5px;border: 1px solid #ccc;">
                          <div class="up">
                            <div class="headimg" style="float: left;padding-right: 5px;">
                              <img :src="user.user_pic" width="50" height="65">
                            </div>
                            <div class="sign">
                              考生签字：
                            </div>
                          ...
                          <div class="down">
                            ...
                            <p>准考证号:{{user.user_admission_no}}</p>
                            <p>身份证号:</p>
                            <p>{{user.user_idcard}}</p>
                            <p>岗位(代码):{{user.user_pos_name}}</p>
                          </div>
                        </div>
...
export default {
  ...
  methods: {
    ...
    checkList(item, room){
      ...
      // 分组
      let groups = this.groupArray(room.users, 15);
      room.groups = groups;
    ...
    getUserList(exam_no){
        ...
        this.$db.all(SQL, (err, res) => {
          ...
          for (var i = 0; i < json.length; i++) {
            for (var j = 0; j < json[i].room_users.length; j++) {
              let groups = this.groupArray(json[i].room_users[j].users, 15);
              json[i].room_users[j].groups = groups;              
            }            
          }
